...
- [x] 5.4 With explicit approval, train one bounded ordinary-loss pilot, capture loss/RSS/time/artifact metadata, and verify that every spawned process stops.
- [x] 5.5 Evaluate the ordinary-loss pilot on the unchanged complete suite and record whether copy bias or missed-edit slices justify an edit-aware objective.
...
- **5.4-5.5 complete (2026-07-25), pilot REGRESSED:** owner-approved, GPU lock held and released, no lingering process. Trained on the manifest's 12 train-split rows only; the 4 development rows were monitored, never trained on. Scored on the unchanged frozen `eval-v1.jsonl` through the shared foundation evaluator. Result: error reduction **-0.8125** against a **+0.0625** zero-shot baseline (delta -0.875) — the candidate makes text worse than the noisy input. Unnecessary edit rate 0.839 against a 0.005 bar; protected-span preservation fell 0.867 -> 0.800. Evidence: `evals/autocorrect/pilot-result-v1.json`.
- **Failure mode is overcorrection, not copy bias.** The model became a paraphraser: `remeber` -> `remind you`, `teh team` -> `your team`, `tomorroww` -> `tomorrow morning`, while `repourt` was left unrepaired. Structural slices are clean (casing, name, number, url all 1.0 error reduction, zero unnecessary edits); the regression is concentrated in lexical repair turning into semantic rewriting, which the design's non-goals explicitly forbid.
- **Recipe defect found — unsatisfiable stop rule:** `thresholds-v1.stop_on_clean_preservation_below` is 0.995 but the base's own zero-shot clean preservation is 0.6667, so the rule fires at the first scheduled evaluation regardless of training. The pilot therefore ran 50 of 300 steps by construction. A ship-grade regression bar is being used as a training stop rule. This pilot's regression is real and measured, but it is truncated evidence, not a fair test of the 300-step recipe. Fixing it requires a new thresholds/recipe version.
- **5.6-5.7 NOT justified (recorded 5.5 answer):** an edit-aware objective up-weights positions needing correction, which targets the opposite of the measured failure and would likely raise overcorrection past task 5.7's explicit reject condition. Copy bias was the pre-training behaviour, not the post-training one. The blocking problems are 12 training rows, the unsatisfiable stop rule, and the absence of any meaning-change guard — none of which an edit-aware loss addresses.
- **6.1-6.4 and 7.1-7.6** remain gated as before; no candidate is near a decision.
