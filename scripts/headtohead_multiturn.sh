...
#
#   - local mlx candidates  -> scripts/bfcl_multiturn_eval.py     (MODEL=<path>)
#   - OpenAI-endpoint ones  -> scripts/bfcl_multiturn_deepseek.py (DS_URL + DS_MODEL)
#     (frontier = api.deepseek.com; Gemma = LM Studio at :1234)
#
# Edit CANDIDATES then: scripts/headtohead_multiturn.sh [n=12] [data_tag=hard]
set -uo pipefail
N="${1:-12}"; TAG="${2:-hard}"
...
for c in "${CANDIDATES[@]}"; do
  ...
  if [ "$kind" = local ]; then
    pct=$(MODEL="$ref" MT_SYS="$PLAN_PROMPT" MT_DATA="$DATA" MT_GOLD="$GOLD" \
          python3 scripts/bfcl_multiturn_eval.py "$N" 2>/dev/null | grep -oE "= [0-9.]+% ==" | grep -oE "[0-9.]+")
  else
    keyarg="/tmp/deepseek_key"; [ "$url" = "$LMS" ] && keyarg="/tmp/_no_key"
    pct=$(DS_KEY_FILE="$keyarg" DS_URL="$url" DS_MODEL="$ref" MT_SYS="$PLAN_PROMPT" \
          MT_DATA="$DATA" MT_GOLD="$GOLD" python3 scripts/bfcl_multiturn_deepseek.py "$N" 2>/dev/null \
