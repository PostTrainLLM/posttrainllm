"""Publish the Fine-Tune Report Card cohort into the public site.
    ...
    python3 scripts/publish_report_cards.py --check    # fail if committed cards drifted
...
- `qwen06-sql-routed-v1` is rendered from the committed SQL fixtures by
  `scripts/render_sql_factory_run.py` into a throwaway run folder, then
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if drifted:
            ...
            print(
                "\nRegenerate with: python3 scripts/publish_report_cards.py",
                file=sys.stderr,
            )
            return 1
