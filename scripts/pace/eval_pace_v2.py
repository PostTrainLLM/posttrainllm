...
Usage:
  ...
  python3 scripts/pace/eval_pace_v2.py --serve-url http://127.0.0.1:8765/v1/chat/completions

  # Custom fixtures dir (e.g. v1 to see the false-positive comparison):
  python3 scripts/pace/eval_pace_v2.py --fixtures-dir /Users/sarthak/Desktop/fleet/clickyLocal/evals/fm-fixtures
"""
...
DEFAULT_SCHEMA = Path(
    "/Users/sarthak/Desktop/fleet/posttrainllm/grammars/pace-fm-label-response.schema.json"
)
...
def query_serve(
    url: str, model_id: str, sys_prompt: str, schema: dict, fx: dict, timeout: int = 180
) -> str:
    body = {
        "model": model_id,
        "messages": [
...
def run(
    ...
    for fx_path in fxs:
        ...
        row = {
            ...
            "fakepace_passed": fp_passed,
            "fakepace_response": fp_resp,
            "fakepace_reasons": fp_reasons,
            "model_passed": mdl_passed,
            "model_response": mdl_resp,
        ...
        if serve_url:
            mdl_mark = (
                "PASS" if mdl_passed else ("FAIL" if mdl_passed is not None else "skip")
            )
            delta = (
                "  +1"
                if (mdl_passed and not fp_passed)
                else (
                    "  -1"
                    if (fp_passed and not mdl_passed)
                    else ("   =" if (mdl_passed == fp_passed) else "    ")
                )
            )
    ...
    if serve_url:
        ...
        only_mdl = sum(
            1 for r in rows if r["model_passed"] and not r["fakepace_passed"]
        )
        both = sum(1 for r in rows if r["fakepace_passed"] and r["model_passed"])
        neither = sum(
            1 for r in rows if not r["fakepace_passed"] and not r["model_passed"]
        )
        print(f"Model score:        {mdl_pass}/{n}  ({100 * mdl_pass / n:.1f}%)")
        print()
        delta_pp = 100 * (mdl_pass - fp_pass) / n
        print(
            f"Δ Model − FakePace: {mdl_pass - fp_pass:+d} fixtures  ({delta_pp:+.1f} pp)"
        )
        print()
    ...
    return {
        ...
        "model_pass": (
            sum(1 for r in rows if r["model_passed"]) if serve_url else None
        ),
        "rows": rows,
    }
...
def main() -> None:
    ...
    p.add_argument(
        "--model", default="posttrainllm", help="model ID for the request body"
    )
