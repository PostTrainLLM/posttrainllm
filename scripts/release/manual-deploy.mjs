...
if (behind !== 0 || ahead !== 0) {
  fail(
    `branch is not synced with ${upstream}: ahead ${ahead}, behind ${behind}`,
  );
}
...
if (ci.status !== "completed" || ci.conclusion !== "success") {
  fail(
    `${ciWorkflowFile} is not green: ${ci.status}/${ci.conclusion || "none"} ${ci.url || ""}`,
  );
}

console.log(`Dispatching ${workflowFile} for ${slug}@main`);
execFileSync("gh", ["workflow", "run", workflowFile, "--ref", "main"], {
  stdio: "inherit",
});
