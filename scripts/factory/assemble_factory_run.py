...
Usage:
    ...
    python3 scripts/factory/assemble_factory_run.py runs/<id>
...
def build_provenance(run_dir: Path, payloads: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "renderer": "scripts/factory/assemble_factory_run.py",
        "renderer_command": f"python3 scripts/factory/assemble_factory_run.py {run_dir}",
        "git": {
...
def main() -> int:
    ...
    if args.publish_check:
        checker = ROOT / "scripts/factory/check_factory_run_publish.py"
        rc = subprocess.call(
