...
#
# Runs (in order):
#   1. scripts/pipelines/eval_planner.sh <model> <tag>        # unhappy-paths n=130
#   2. posttrainllm eval-bfcl --base-url … --out …       # BFCL 10 categories
#   3. posttrainllm eval-tau-bench --base-url … --out …  # retail + airline
#   4. scripts/bench/bench_decode.py --url … --model …    # decode + RSS
#
...
#
#   scripts/bench/build_slm_leaderboard.py \
...
# --- 1. unhappy-paths ---------------------------------------------------
skip_or_run "unhappy" "$UNHAPPY_JSON" bash "$REPO/scripts/pipelines/eval_planner.sh" "$MODEL" "$TAG"
...
skip_or_run "decode" "$DECODE_JSON" \
  bash -c "python3 '$REPO/scripts/bench/bench_decode.py' \
...
EOF
echo "  then: python3 scripts/bench/build_slm_leaderboard.py --manifest docs/research/data/leaderboard_manifest.json"
