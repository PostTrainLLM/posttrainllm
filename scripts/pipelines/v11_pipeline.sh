...
#
# Usage:
#   bash scripts/pipelines/v11_pipeline.sh                # train on whatever corpus exists
#   bash scripts/pipelines/v11_pipeline.sh --amplify     # run the thinking-teacher amplifier first
...
run_dim () {  # name, fixtures-dir
  ...
  python3 "$TGT/scripts/pace/eval_pace_v2.py" \
...
run_unhappy () {  # name, fixtures-dir
  echo "--- $1 ---" | tee -a "$EVAL_LOG"
  python3 "$TGT/scripts/pace/eval_pace_unhappy.py" \
    --fixtures-dir "$2" --serve-url "$SERVE_URL" --sys-prompt "$SYSP" \
...
python3 "$TGT/scripts/bfcl/eval_bfcl.py" \
  --serve-url "$SERVE_URL" \
