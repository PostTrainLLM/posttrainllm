#!/usr/bin/env bash
# scripts/pipelines/score-run.sh — score every history checkpoint of a training run.
#
# Usage:
#   ./scripts/pipelines/score-run.sh <canonical.tinygpt> [out.jsonl]
...
for c in "${ALL_CKPTS[@]}"; do
    ...
    "$REPO_ROOT/scripts/pipelines/score-checkpoint.sh" "$c" "$OUT" || {
