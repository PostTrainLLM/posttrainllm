...
#
#   scripts/pipelines/eval_planner.sh <lm-studio-model-id> [run-tag] [--compact]
#
...
if ! bash "$TGT/scripts/pipelines/eval_combined.sh" "$TAG" "$URL" "$MODEL" "$SYSP" "$TAG"; then
...
if [ "$COMPACT" -eq 1 ]; then
  ...
  if ! bash "$TGT/scripts/pipelines/eval_combined.sh" "$TAG_B" "$URL" "$MODEL" "$SYSP_COMPACT" "$TAG_B"; then
    echo "[eval-planner] compact eval aborted for $MODEL — A/B incomplete." >&2
    exit 1
  fi
  python3 "$TGT/scripts/pipelines/eval_planner_report.py" "$TAG" --candidate-b "$TAG_B"
else
  python3 "$TGT/scripts/pipelines/eval_planner_report.py" "$TAG"
fi
