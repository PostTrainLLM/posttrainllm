...
Usage: python3 scripts/archive/build-clarify-seeds.py --out ~/.cache/posttrainllm/datasets/clarify-seeds.jsonl
"""
    ...
    ROWS.append(
        {
            ...
            "response": response,
            "_meta": {
                "intent_class": "clarify" if kind == "ambiguous" else "action",
                "contrast_pair": scenario,
                "kind": kind,
            },
        }
    )
...
MULTI = [
    (
        ...
        "file",
        [
            ("Q3-board-report.pdf", "report"),
            ("metrics-report.pdf", "report"),
            ("readme.md", "doc"),
        ],
        [
            ("Q3-board-report.pdf", "report"),
            ("holiday-photos.zip", "archive"),
            ("readme.md", "doc"),
        ],
    ),
    (
    ...
    (
        ...
        "row",
        [
            ("Jordan Lee — mobile", "contact"),
            ("Jordan Smith — work", "contact"),
            ("Settings", "menu"),
        ],
    ...
    (
        ...
        "row",
        [
            ("Standup — 9:30", "meeting"),
            ("Architecture sync — 9:30", "meeting"),
            ("Calendar", "app"),
        ],
        [
            ("Standup — 9:30", "meeting"),
            ("Tomorrow: 1:1", "meeting tomorrow"),
            ("Calendar", "app"),
        ],
    ),
    (
    ...
    (
        "terminal tab",
        "kill the server in the terminal",
        "tab",
        [
            ("dev-server :3000", "session"),
            ("dev-server :8080", "session"),
            ("logs", "session"),
        ],
        [
            ("dev-server :3000", "session"),
    ...
    (
        ...
        "row",
        [
            ("invoice-march.pdf", "attachment"),
            ("invoice-april.pdf", "attachment"),
            ("signature.png", "inline"),
        ],
        [
            ("invoice-march.pdf", "attachment"),
            ("Reply", "button"),
...
for scenario, user, role, ambiguous_set, unambiguous_set in MULTI:
    amb_els = [
        el(i, role, f"{100 + i * 60},200", lab, txt)
        for i, (lab, txt) in enumerate(ambiguous_set)
    ]
    ...
    add(
        ...
        amb_els,
        clarify(
            f"i see more than one — {first_two}?", f"which one: {first_two}?", "target"
        ),
        scenario,
    )
    un_els = [
        el(i, role, f"{100 + i * 60},200", lab, txt)
        for i, (lab, txt) in enumerate(unambiguous_set)
    ]
    add(
        "unambiguous",
        user,
        un_els,
        action("on it", "AX.press", {"target": unambiguous_set[0][0]}),
        scenario,
    )
...
MISSING = [
    (
        "mail-recipient",
        "email the budget sheet",
        "email the budget sheet to finance@acme.com",
        "who should i send it to?",
        "recipient",
        "Mail.draft",
        {"to": ["finance@acme.com"], "subject": "Budget sheet"},
    ),
    (
        "event-time",
        "schedule a sync with the design team",
        "schedule a sync with the design team thursday at 3pm",
        "when should i put it?",
        "time",
        "Cal.event",
        {"title": "Sync with design team", "start": "thursday 3pm"},
    ),
    (
        "reminder-time",
        "remind me to renew the passport",
        "remind me tomorrow morning to renew the passport",
        "for when?",
        "time",
        "Reminders.add",
        {"title": "Renew the passport", "due": "tomorrow morning"},
    ),
    (
        "note-content",
        "start a note for the offsite",
        "start a note for the offsite: agenda, budget, travel",
        "what should go in it?",
        "content",
        "Notes.create",
        {"title": "Offsite", "body": "agenda, budget, travel"},
    ),
    (
        "message-body",
        "reply to dana",
        "reply to dana saying i'll review it tonight",
        "what should i say?",
        "content",
        "Mail.draft",
        {"to": ["__resolve:dana"], "body": "I'll review it tonight."},
    ),
    (
        "timer-duration",
        "start a countdown",
        "start a twenty minute countdown",
        "how long?",
        "duration",
        "Shortcut.run",
        {"name": "Timer", "input": "20 minutes"},
    ),
]

for scenario, amb_user, clear_user, q, topic, name, args in MISSING:
    add("ambiguous", amb_user, [], clarify(q, q, topic), scenario)
    add("unambiguous", clear_user, [], action("done", name, args), scenario)
...
for scenario, user, role, _, referent_set in PRONOUN:
    add(
        ...
        user,
        [],
        clarify("which one do you mean?", "which item should i use?", "target"),
        scenario,
    )
    ...
    add(
        ...
        user,
        ref_els,
        action("done", "AX.press", {"target": referent_set[0][0]}),
        scenario,
    )
...
def main() -> int:
    ...
    print(
        f"wrote {len(ROWS)} seeds ({amb} clarify / {len(ROWS) - amb} action twins) -> {args.out}"
    )
    return 0
