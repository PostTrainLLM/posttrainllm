...
sleep 3
bash "$TGT/scripts/pipelines/eval_combined.sh" two-stage-4b http://127.0.0.1:8769/v1/chat/completions \
...
for MODEL in \
  ...
  bash "$TGT/scripts/pipelines/eval_combined.sh" "$TAG" http://127.0.0.1:1234/v1/chat/completions "$MID" "$SYSP" "$TAG"
done
