#!/usr/bin/env bash
# scripts/archive/migrate-tmp-runs.sh — one-time helper to move surviving /tmp
# training artifacts into ~/.cache/posttrainllm/runs/<name>/.
#
# Usage:
#   ./scripts/archive/migrate-tmp-runs.sh          # interactive
#   ./scripts/archive/migrate-tmp-runs.sh --yes    # move all without prompting
...
This run was moved by \`scripts/archive/migrate-tmp-runs.sh\` on $(date -u +%Y-%m-%dT%H:%M:%SZ).
