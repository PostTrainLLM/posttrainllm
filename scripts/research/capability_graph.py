...
def exact_fields(
    value: Any, required: set[str], path: str, errors: list[str]
) -> dict[str, Any] | None:
...
def scan_private(
    value: Any, denylist: list[str], errors: list[str], path: str = "$"
) -> None:
    if isinstance(value, dict):
        ...
        r"(?:sk|key|token)[-_][A-Za-z0-9]{16,}", value, re.I
    ):
...
def validate_policy(policy: dict[str, Any]) -> list[str]:
    ...
    if (
        not isinstance(policy.get("max_hops"), int)
        or isinstance(policy.get("max_hops"), bool)
        or policy["max_hops"] < 1
    ):
    ...
    if (
        not isinstance(policy.get("allowed_privacy_classes"), list)
        or not policy["allowed_privacy_classes"]
    ):
...
def validate_graph(graph: dict[str, Any], *, root: Path = ROOT) -> list[str]:
    ...
    for index, raw in enumerate(nodes_raw):
        ...
        identities = [
            nonempty(node.get("package_id")),
            nonempty(node.get("runtime_id")),
        ]
        if sum(identities) != 1:
            errors.append(
                f"{path}: exactly one of package_id or runtime_id is required"
            )
        ...
        envelope = exact_fields(
            node.get("operating_envelope"),
            {"eval_ref", "quality", "ood_behavior"},
            f"{path}.operating_envelope",
            errors,
        )
        if envelope:
            if not nonempty(envelope.get("eval_ref")) or not nonempty(
                envelope.get("ood_behavior")
            ):
                errors.append(
                    f"{path}.operating_envelope: eval_ref and ood_behavior are required"
                )
            if (
                not finite_number(envelope.get("quality"))
                or not 0 <= envelope["quality"] <= 1
            ):
                errors.append(
                    f"{path}.operating_envelope.quality: must be between 0 and 1"
                )
        verifier = exact_fields(
            node.get("verifier_policy"),
            {"kind", "verifier_id", "rule"},
            f"{path}.verifier_policy",
            errors,
        )
        if verifier:
            if verifier.get("kind") not in schema["verifier_kinds"]:
                errors.append(f"{path}.verifier_policy.kind: unsupported")
            if not nonempty(verifier.get("verifier_id")) or not nonempty(
                verifier.get("rule")
            ):
                errors.append(
                    f"{path}.verifier_policy: verifier_id and rule are required"
                )
            if (
                node.get("kind") in {"specialist", "generalist", "external-fallback"}
                and "self-confidence" in str(verifier.get("rule", "")).lower()
            ):
                errors.append(
                    f"{path}.verifier_policy: self-confidence cannot accept a generative result"
                )
        privacy = exact_fields(
            node.get("privacy"), {"class", "network_allowed"}, f"{path}.privacy", errors
        )
        if privacy and (
            not nonempty(privacy.get("class"))
            or not isinstance(privacy.get("network_allowed"), bool)
        ):
            errors.append(f"{path}.privacy: invalid privacy contract")
        resources = node.get("resources")
        if not isinstance(resources, dict) or set(resources) != set(
            schema["required_resources"]
        ):
            errors.append(
                f"{path}.resources: must contain exactly the required measurements"
            )
        else:
            for name, raw_measurement in resources.items():
                measurement = exact_fields(
                    raw_measurement,
                    {"state", "value", "unit", "source"},
                ...
                if state in {"measured", "derived", "historical"}:
                    if (
                        not finite_number(measurement.get("value"))
                        or measurement["value"] < 0
                    ):
                        errors.append(
                            f"{path}.resources.{name}.value: measured value must be non-negative"
                        )
                    if not nonempty(measurement.get("source")):
                        errors.append(
                            f"{path}.resources.{name}.source: evidence source required"
                        )
                elif measurement.get("value") is not None:
                    errors.append(
                        f"{path}.resources.{name}.value: must be null for {state}"
                    )
        ...
        if node.get("kind") == "external-fallback":
            ...
            if not (privacy or {}).get("network_allowed"):
                errors.append(
                    f"{path}: external fallback must declare its network boundary"
                )
    ...
    for index, raw in enumerate(edges_raw):
        ...
        if edge.get("from") not in nodes or edge.get("to") not in nodes:
            ...
            continue
        if (
            not isinstance(edge.get("order"), int)
            or isinstance(edge.get("order"), bool)
            or edge["order"] < 1
        ):
        ...
        if edge.get("kind") in {"routes-to", "fallback-to"}:
            if target["request_schema"] != edge.get("request_schema") or target[
                "response_schema"
            ] != edge.get("response_schema"):
    ...
    verified_sources = {
        edge["from"]
        for edge in edges
        if edge.get("kind") == "verified-by" and edge.get("executable")
    }
    fallback_sources = {
        edge["from"]
        for edge in edges
    ...
    for node_id, node in nodes.items():
        if node["kind"] in {"specialist", "generalist", "external-fallback"}:
            ...
            if (
                verifier_id not in nodes
                or nodes[verifier_id]["kind"] != "verifier"
                or node_id not in verified_sources
            ):
                errors.append(
                    f"$.nodes[{node_id}]: generative node has no valid verifier edge"
                )
            if (
                node_id not in fallback_sources
                and node["kind"] != "external-fallback"
                and "safe-failure" not in node["terminal_behavior"]
            ):
...
def fallback_path(graph: dict[str, Any], start: str) -> list[str]:
    ...
    while by_source.get(current):
        edge = sorted(by_source[current], key=lambda item: (item["order"], item["id"]))[
            0
        ]
        current = edge["to"]
        if current in seen:
...
def residency_plan(
    ...
    capacity = min(
        int(snapshot.get("capacity_bytes", policy["max_resident_bytes"])),
        int(policy["max_resident_bytes"]),
    )
    residents = sorted(
        snapshot.get("resident", []),
        key=lambda item: (item["last_used"], item["node_id"]),
    )
    ...
    if load_bytes is None:
        return {
            ...
            "after_bytes": None,
            "feasible": False,
            "reason": "resource-evidence-missing",
        }
    evictions = []
    remaining = used
    ...
    return {
        "capacity_bytes": capacity,
        "before_bytes": used,
        "load_bytes": int(load_bytes),
        "evictions": evictions,
        "after_bytes": int(remaining + load_bytes) if feasible else None,
        "feasible": feasible,
    }
...
def dry_run(
    ...
    if graph_errors or policy_errors:
        raise ValueError(
            "invalid graph/policy: " + "; ".join(graph_errors + policy_errors)
        )
    ...
    bypass = (
        confidence < policy["min_route_confidence"]
        or ood_score > policy["max_route_ood_score"]
    )
    ...
    for node_id in ranked:
        ...
        if node["kind"] not in {"specialist", "generalist", "external-fallback"}:
            ...
            reason = (
                ...
                if confidence < policy["min_route_confidence"]
                else "route-out-of-distribution"
            )
            ...
            request["capability"] not in node["capabilities"]
            and "*" not in node["capabilities"]
        ):
            reason = "capability-mismatch"
        elif (
            request["request_schema"] != node["request_schema"]
            or request["response_schema"] != node["response_schema"]
        ):
            reason = "schema-mismatch"
        elif request["privacy_class"] not in policy["allowed_privacy_classes"]:
            reason = "privacy-policy-blocked"
        elif (
            node["kind"] != "external-fallback"
            and request["privacy_class"] == "local-private"
            and node["privacy"]["class"] != "local-private"
        ):
            reason = "privacy-policy-blocked"
        elif (
            node["kind"] != "external-fallback"
            and node["privacy"]["network_allowed"]
            and not policy["allow_network"]
        ):
            reason = "network-not-authorized"
        elif node["kind"] == "external-fallback" and (
            not policy["allow_external"]
            or not policy["allow_network"]
            or not request.get("external_authorized", False)
        ):
            reason = "network-not-authorized"
    ...
    if selected and not residency["feasible"]:
        exclusions.append(
            {
                "node_id": selected,
                "reason": residency.get("reason", "resource-budget-exhausted"),
            }
        )
    ...
    return {
        ...
        "router": {
            ...
            "ood_score": ood_score,
            "bypassed_specialists": bypass,
        },
        "candidates": ranked,
        "eligible": eligible,
        "exclusions": sorted(
            exclusions, key=lambda item: (item["node_id"], item["reason"])
        ),
...
def cascade(
    ...
    chain = (
        ([route["selected_node"]] + route["fallback_path"])
        if route["selected_node"]
        else []
    )
    attempts = []
    total_latency = 0.0
    ...
    for node_id in chain:
                ...
                attempts.append(
                    {
                        ...
                        "status": "blocked",
                        "accepted": False,
                        "failure": "network-not-authorized",
                        "latency_ms": 0,
                        "verifier": None,
                        "output_sha256": None,
                    }
        ...
        verifier = (
            outcome.get("verifier") if outcome.get("status") == "success" else None
        )
        ...
        if (
            ...
            or external_cost > policy["max_external_cost_usd"]
        ):
    ...
    if accepted is None:
        ...
        terminal = {
            "kind": "accepted",
            "failure": None,
            "accepted_node": accepted["node_id"],
        }
    peak_resident = route["residency"].get("after_bytes")
    active_values = [
        resource_value(nodes[item["node_id"]], "active_parameters") for item in attempts
    ]
    installed_values = [
        resource_value(nodes[item["node_id"]], "installed_artifact_bytes")
        for item in attempts
    ]
    shared_values = [
        resource_value(nodes[item["node_id"]], "shared_base_bytes") for item in attempts
    ]
    adapter_values = [
        resource_value(nodes[item["node_id"]], "adapter_bytes") for item in attempts
    ]
    trace = {
        ...
        "resources": {
            ...
            "latency_mode": "warm"
            if route["residency"].get("load_bytes") == 0
            else "cold",
            "loaded_bytes": route["residency"].get("load_bytes"),
            "peak_resident_bytes": peak_resident,
            "max_active_parameters": max(
                (value for value in active_values if value is not None), default=None
            ),
            "installed_bytes_touched": sum(
                value for value in installed_values if value is not None
            ),
            "shared_base_bytes_touched": max(
                (value for value in shared_values if value is not None), default=None
            ),
            "adapter_bytes_touched": sum(
                value for value in adapter_values if value is not None
            ),
    ...
    if private_errors:
        raise ValueError(
            "trace privacy validation failed: " + "; ".join(private_errors)
        )
    return {
        "contract_version": "specialist-capability-execution/v1",
        "result": result,
        "trace": trace,
    }
...
def benchmark_prediction(
    ...
    output = {
        ...
        "routing": {
            "eligible_nodes": trace["routing"]["eligible"]
            or [trace["routing"]["selected_node"]],
            ...
            "binding": {
                ...
                "invoked_package_ids": [
                    item["package_id"] or item["runtime_id"] for item in attempts
                ],
                "verifier_ids": [
                    item["verifier"]["id"]
                    for item in attempts
    ...
    return {
        ...
        "instance_set_ref": {
            "id": task["instance_set"]["id"],
            "revision": task["instance_set"]["revision"],
        },
        "outputs": [output],
    }
...
def system_qualification(
    final_accuracy: float, metrics: dict[str, Any], gates: dict[str, Any]
) -> dict[str, Any]:
    required_gates = {
        "contract_version",
        "gate_id",
        ...
        "min_route_accuracy",
        "min_escalation_recall",
        "max_over_escalation_rate",
        "max_latency_end_to_end_ms_mean",
        "max_external_calls",
        "max_external_cost_usd",
    }
    ...
    observed = {
        ...
        "over_escalation_rate": metrics.get("over_escalation_rate")
        if isinstance(metrics, dict)
        else None,
        "latency_end_to_end_ms_mean": resources.get("latency_end_to_end_ms_mean")
        if isinstance(resources, dict)
        else None,
        "external_calls": resources.get("external_calls")
        if isinstance(resources, dict)
        else None,
        "external_cost_usd": resources.get("external_cost_usd")
        if isinstance(resources, dict)
        else None,
    }
    comparisons = {
    ...
    for name, (direction, threshold) in comparisons.items():
        value = observed[name]
        passed = finite_number(value) and (
    ...
    try:
        if args.command == "validate":
            ...
            nodes = [
                ...
                if args.capability in node["capabilities"]
            ...
            request, installed, router = (
                load(args.request),
                load(args.installed),
                load(args.router_output),
            )
            value = dry_run(graph, policy, request, installed, router)
            if args.command == "cascade":
                value = cascade(
                    graph, policy, request, installed, router, load(args.outcomes)
                )
            write_or_print(value, args.out)
        else:
            value = benchmark_prediction(
                load(args.execution),
                load(args.task),
                load(args.entry),
                args.instance_id,
