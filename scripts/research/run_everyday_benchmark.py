...
def load_and_validate(
    path: Path, expected_type: str, contract: dict[str, Any]
) -> dict[str, Any]:
    ...
    if value.get("artifact_type") != expected_type:
        raise ValueError(
            f"{path}: expected {expected_type}, found {value.get('artifact_type')!r}"
        )
    return value
...
def assert_identity(
    ...
    if predictions["instance_set_ref"] != expected_instance_ref:
        raise ValueError(
            "prediction instance_set_ref does not match the selected instance set"
        )
    task_instance = (
        task["instance_set"]
        if instances["layer"] == "public-development"
        else task["official_instance_set"]
    )
    ...
    if instances["layer"] == "sealed-official":
        ...
        if task_instance["sha256"] != actual_hash or task_instance["count"] != len(
            instances["instances"]
        ):
            raise ValueError(
                "sealed instance set does not match the task's frozen hash/count"
            )


def assert_complete_predictions(
    task: dict[str, Any], instances: dict[str, Any], predictions: dict[str, Any]
) -> None:
    ...
    actual = {
        (output["instance_id"], output["pass_index"])
...
def with_derived_resources(
    resources: list[dict[str, Any]], outputs: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def system_metrics(
    outputs: list[dict[str, Any]], expected_by_id: dict[str, str]
) -> dict[str, Any]:
    false_accepts = 0
    accepted = 0
    ...
    for output in outputs:
        ...
        if (
            ...
            or not required.issubset(routing)
            or set(routing) - required - optional
        ):
            raise ValueError(
                f"system output {output['instance_id']} must provide the exact routing trace contract"
            )
        if (
            not isinstance(routing["eligible_nodes"], list)
            or not routing["eligible_nodes"]
        ):
            raise ValueError(
                f"system output {output['instance_id']} eligible_nodes must be non-empty"
            )
        if (
            routing["selected_node"] not in routing["eligible_nodes"]
            or routing["best_eligible_node"] not in routing["eligible_nodes"]
        ):
            raise ValueError(
                f"system output {output['instance_id']} selected/best node is not eligible"
            )
        if (
            not checker.is_number(routing["route_regret"])
            or routing["route_regret"] < 0
        ):
            raise ValueError(
                f"system output {output['instance_id']} route_regret must be non-negative"
            )
        if (
            not isinstance(routing["hops"], int)
            or isinstance(routing["hops"], bool)
            or routing["hops"] < 1
        ):
            raise ValueError(
                f"system output {output['instance_id']} hops must be positive"
            )
        if not all(
            isinstance(routing[field], bool)
            for field in ("accepted", "escalated", "should_escalate")
        ):
            raise ValueError(
                f"system output {output['instance_id']} routing flags must be booleans"
            )
        if "binding" in routing:
            ...
            if not isinstance(binding, dict) or set(binding) != required_binding:
                raise ValueError(
                    f"system output {output['instance_id']} graph binding is invalid"
                )
            if (
                not isinstance(binding["invoked_node_ids"], list)
                or not binding["invoked_node_ids"]
            ):
                raise ValueError(
                    f"system output {output['instance_id']} invoked_node_ids must be non-empty"
                )
            if not isinstance(binding["invoked_package_ids"], list) or len(
                binding["invoked_package_ids"]
            ) != len(binding["invoked_node_ids"]):
                raise ValueError(
                    f"system output {output['instance_id']} invoked_package_ids must align with invoked_node_ids"
                )
            if not isinstance(binding["verifier_ids"], list):
                raise ValueError(
                    f"system output {output['instance_id']} verifier_ids must be an array"
                )
        if "resource_evidence" in routing:
            ...
            required_resources = {
                ...
                "installed_bytes_touched",
                "shared_base_bytes_touched",
                "adapter_bytes_touched",
                "external_calls",
                "external_cost_usd",
            }
            if (
                not isinstance(resource_evidence, dict)
                or set(resource_evidence) != required_resources
            ):
                raise ValueError(
                    f"system output {output['instance_id']} resource_evidence is invalid"
                )
            if resource_evidence["latency_mode"] not in {"cold", "warm"}:
                raise ValueError(
                    f"system output {output['instance_id']} resource_evidence.latency_mode must be cold or warm"
                )
            nullable_resources = {
                "loaded_bytes",
                "peak_resident_bytes",
            ...
            for field in required_resources - {"latency_mode"} - nullable_resources:
                if (
                    not checker.is_number(resource_evidence[field])
                    or resource_evidence[field] < 0
                ):
                    raise ValueError(
                        f"system output {output['instance_id']} resource_evidence.{field} must be non-negative"
                    )
            for field in nullable_resources:
                if resource_evidence[field] is not None and (
                    ...
                    raise ValueError(
                        f"system output {output['instance_id']} resource_evidence.{field} must be null or non-negative"
                    )
            resource_rows.append(resource_evidence)
        prediction_field = next(
            (
                field
                for field in ("predicted_label", "predicted_text", "predicted_verdict")
                if field in output
            ),
            None,
        )
        ...
        first_hop_correct += int(
            not routing["escalated"] and routing["accepted"] and correct
        )
        ...
        escalation_true_positive += int(
            routing["escalated"] and routing["should_escalate"]
        )
    ...
    if resource_rows:
        cold_latencies = [
            row["latency_end_to_end_ms"]
            for row in resource_rows
            if row["latency_mode"] == "cold"
        ]
        warm_latencies = [
            row["latency_end_to_end_ms"]
            for row in resource_rows
            if row["latency_mode"] == "warm"
        ]
        resource_metrics = {
            "latency_end_to_end_ms_mean": statistics.fmean(
                row["latency_end_to_end_ms"] for row in resource_rows
            ),
            "latency_end_to_end_ms_max": max(
                row["latency_end_to_end_ms"] for row in resource_rows
            ),
            "latency_cold_end_to_end_ms_mean": statistics.fmean(cold_latencies)
            if cold_latencies
            else None,
            "latency_warm_end_to_end_ms_mean": statistics.fmean(warm_latencies)
            if warm_latencies
            else None,
            "peak_resident_bytes": max(
                (
                    ...
                    for row in resource_rows
                    if row["peak_resident_bytes"] is not None
                ),
                default=None,
            ),
            "max_active_parameters": max(
                (
                    row["max_active_parameters"]
                    for row in resource_rows
                    if row["max_active_parameters"] is not None
                ),
                default=None,
            ),
            "loaded_bytes_max": max(
                (
                    row["loaded_bytes"]
                    for row in resource_rows
                    if row["loaded_bytes"] is not None
                ),
                default=None,
            ),
            "installed_bytes_touched_max": max(
                row["installed_bytes_touched"] for row in resource_rows
            ),
            "shared_base_bytes_touched_max": max(
                (
                    row["shared_base_bytes_touched"]
                    for row in resource_rows
                    if row["shared_base_bytes_touched"] is not None
                ),
                default=None,
            ),
            "adapter_bytes_touched_max": max(
                row["adapter_bytes_touched"] for row in resource_rows
            ),
...
def score(
    ...
    expected_by_id = {
        instance["id"]: instance[expected_field] for instance in instances["instances"]
    }
    slices_by_id = {
        instance["id"]: instance["slices"] for instance in instances["instances"]
    }
    ...
    for output in predictions["outputs"]:
        ...
        if task["labels"]:
            ...
            confusion["expected-output"][
                "__error__" if failed else ("match" if is_correct else "mismatch")
            ] += 1
        ...
        if (
            task["scorer"].get("unknown_label") is not None
            and expected == task["scorer"]["unknown_label"]
        ):
            unknown_total += 1
    ...
    consistent = sum(
        1 for labels in predictions_by_instance.values() if len(set(labels)) == 1
    )
    ...
    result = {
        ...
        "resources": resources,
        "system_metrics": system_metrics(predictions["outputs"], expected_by_id)
        if entry["track"] == "system"
        else None,
...
def load_official_metadata(path: Path | None, layer: str) -> dict[str, Any] | None:
    if layer != "sealed-official":
        if path is not None:
            raise ValueError(
                "--official-metadata is only valid for sealed-official runs"
            )
        return None
    if path is None:
    ...
    if not isinstance(value, dict) or set(value) != required:
        raise ValueError(
            f"{path}: official metadata must contain exactly {sorted(required)}"
        )
    for field in required - {"overlap_count", "attestation"}:
        if not isinstance(value[field], str) or not value[field].strip():
            raise ValueError(f"{path}: {field} must be a non-empty string")
    if (
        not isinstance(value["overlap_count"], int)
        or isinstance(value["overlap_count"], bool)
    ...
    if attestation["value"] is not None and (
        not isinstance(attestation["value"], str) or not attestation["value"].strip()
    ):
        raise ValueError(
            f"{path}: attestation.value must be null or a non-empty string"
        )
    return value
...
def main() -> int:
    ...
    parser.add_argument(
        "--timestamp",
        required=True,
        help="Caller-supplied ISO timestamp for reproducible fixtures",
    )
    parser.add_argument("--out-dir", required=True, type=Path)
    parser.add_argument(
        "--official-metadata",
        type=Path,
        help="Private custody/leakage metadata for sealed-official runs",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate and score without writing artifacts",
    )
    ...
    try:
        ...
        official_metadata = load_official_metadata(
            args.official_metadata, instances["layer"]
        )
        bundle_errors: list[str] = []
        checker.validate_bundle(
            [suite, task, entry, instances, predictions], contract, bundle_errors
        )
        if bundle_errors:
        ...
        if errors:
            raise ValueError(
                "generated artifact validation failed: " + "; ".join(errors)
            )
        if not args.dry_run:
