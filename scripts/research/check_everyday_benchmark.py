...
def expect_string(
    value: Any, path: str, errors: list[str], *, nullable: bool = False
) -> None:
...
def expect_enum(
    value: Any, allowed: Iterable[str], path: str, errors: list[str]
) -> None:
...
def validate_measurements(
    value: Any, contract: dict[str, Any], path: str, errors: list[str]
) -> None:
    ...
    for index, raw in enumerate(value):
        ...
        item = expect_fields(
            raw, item_path, ("name", "state", "value", "unit", "source"), errors
        )
        if not item:
            continue
        name = item.get("name")
        ...
        if state in {"measured", "derived", "historical"}:
            if not is_number(value_field) or value_field < 0:
                add(
                    errors,
                    f"{item_path}.value",
                    "must be a finite non-negative number for this state",
                )
        elif (
            state in {"skipped", "missing", "not-applicable"}
...
def validate_suite(
    value: dict[str, Any], _contract: dict[str, Any], errors: list[str]
) -> None:
    ...
    publication = expect_fields(
        ...
        "$.publication",
        (
            "minimum_qualified_task_families",
            "headline_requires_same_instance_set",
            "default_view",
    ...
    if publication:
        ...
        if not isinstance(minimum, int) or isinstance(minimum, bool) or minimum < 1:
            add(
                errors,
                "$.publication.minimum_qualified_task_families",
                "must be a positive integer",
            )
        expect_bool(
            publication.get("headline_requires_same_instance_set"),
            "$.publication.headline_requires_same_instance_set",
            errors,
        )
        expect_string(
            publication.get("default_view"), "$.publication.default_view", errors
        )
        expect_bool(
            publication.get("composite_score_authoritative"),
            "$.publication.composite_score_authoritative",
            errors,
        )


def validate_task(
    value: dict[str, Any], contract: dict[str, Any], errors: list[str]
) -> None:
    ...
    adapter = expect_fields(
        value.get("adapter"),
        "$.adapter",
        ("id", "revision", "instruction_ref", "input_field", "output_field"),
        errors,
    )
    if adapter:
    ...
    if scorer:
        for field, item in scorer.items():
            expect_string(
                item, f"$.scorer.{field}", errors, nullable=field == "unknown_label"
            )
    instance_set = expect_fields(
        value.get("instance_set"),
        "$.instance_set",
        ("id", "revision", "path", "layer"),
        errors,
    )
    if instance_set:
        ...
        expect_enum(
            instance_set.get("layer"),
            enums["evaluation_layer"],
            "$.instance_set.layer",
            errors,
        )
    ...
    if official:
        for field in ("id", "revision", "sha256", "custody_receipt_ref"):
            expect_string(
                official.get(field), f"$.official_instance_set.{field}", errors
            )
        ...
        if value.get("status") == "qualified" and not re.fullmatch(
            r"[0-9a-f]{64}", str(official.get("sha256", ""))
        ):
            add(
                errors,
                "$.official_instance_set.sha256",
                "must be a lowercase SHA-256 when the task is qualified",
            )
    ...
    if value.get("task_id") == "pace-intent-routing" and set(labels or []) != set(
        enums["pace_intent_label"]
    ):
        add(errors, "$.labels", "must exactly match the Pace seven-class taxonomy")
    slices = expect_fields(
        value.get("slices"), "$.slices", ("primary", "protected"), errors
    )
    if slices:
        for name in ("primary", "protected"):
            if (
                not isinstance(slices.get(name), list)
                or not slices[name]
                or any(not isinstance(item, str) for item in slices[name])
            ):
    ...
    frontier = expect_fields(
        value.get("frontier_qualification"),
        "$.frontier_qualification",
        ("state", "threshold", "model", "score", "receipt_ref"),
        errors,
    )
    if frontier:
        state = frontier.get("state")
        expect_enum(
            state, enums["frontier_state"], "$.frontier_qualification.state", errors
        )
        ...
        if state == "passed":
            expect_string(
                frontier.get("model"), "$.frontier_qualification.model", errors
            )
            expect_string(
                frontier.get("receipt_ref"),
                "$.frontier_qualification.receipt_ref",
                errors,
            )
            ...
            if not is_number(score) or score < threshold:
                add(
                    errors,
                    "$.frontier_qualification.score",
                    "must meet the threshold when state is passed",
                )
        elif state == "failed":
            expect_string(
                frontier.get("model"), "$.frontier_qualification.model", errors
            ...
            if not is_number(score) or not 0 <= score < threshold:
                add(
                    errors,
                    "$.frontier_qualification.score",
                    "must be below the threshold when state is failed",
                )
        elif any(
            frontier.get(field) is not None
            for field in ("model", "score", "receipt_ref")
        ):
            add(
                errors,
                "$.frontier_qualification",
                "model, score, and receipt_ref must be null when qualification is missing",
            )
    policy = expect_fields(
        value.get("publication_policy"),
        "$.publication_policy",
        (
            "raw_public_development_instances_allowed",
            "raw_sealed_instances_allowed",
            "raw_model_outputs_in_receipt_allowed",
    ...
    if policy:
        ...
        qualified = (
            ...
            and frontier
            and frontier.get("state") == "passed"
        )
        if policy.get("official_ranking_allowed") and not qualified:
            add(
                errors,
                "$.publication_policy.official_ranking_allowed",
                "requires qualified status and passed frontier gate",
            )
    ...
    if kind == "openai-compatible":
        ...
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", obj.get("credential_env", "")):
            add(
                ...
                "must name an environment variable, never a credential value",
            )
...
def validate_entry(
    ...
    if required:
        disclosure = expect_fields(
            value.get("disclosure"), "$.disclosure", required, errors
        )
        if disclosure:
...
def validate_instance_set(
    ...
    expect_enum(
        value.get("layer"), contract["enums"]["evaluation_layer"], "$.layer", errors
    )
    provenance = expect_fields(
        value.get("provenance"),
        "$.provenance",
        ("kind", "generator", "generated_at", "review_status"),
        errors,
    )
    if provenance:
    ...
    for index, raw in enumerate(instances):
        ...
        if len(present) != 1:
            add(
                errors,
                path,
                f"must contain exactly one expected output field from {list(expected_fields)}",
            )
        elif present[0] == "expected_label":
            expect_enum(
                item.get("expected_label"), labels, f"{path}.expected_label", errors
            )
        else:
        ...
        if (
            ...
            or not slices
            or any(not isinstance(part, str) or not part for part in slices)
        ):
            add(errors, f"{path}.slices", "must be a non-empty string array")
        expect_string(
            item.get("boundary_rationale"), f"{path}.boundary_rationale", errors
        )
...
def validate_prediction_set(
    ...
    for index, raw in enumerate(outputs):
        ...
        if error is None:
            if len(present) != 1:
                add(
                    errors,
                    path,
                    f"must contain exactly one predicted output field from {list(prediction_fields)}",
                )
            elif present[0] == "predicted_label":
                expect_enum(
                    item.get("predicted_label"),
                    labels,
                    f"{path}.predicted_label",
                    errors,
                )
            else:
                expect_string(item.get(present[0]), f"{path}.{present[0]}", errors)
            ...
            if any(item.get(field) is not None for field in present):
                add(
                    errors,
                    path,
                    "predicted output must be null or omitted when error is present",
                )
        latency = item.get("latency_ms")
        if not is_number(latency) or latency < 0:
            add(errors, f"{path}.latency_ms", "must be a finite non-negative number")
        if item.get("routing") is not None and not isinstance(
            item.get("routing"), dict
        ):
        ...
        if "decision_signals" in item:
            ...
            if signals:
                expect_string(
                    signals.get("revision"), f"{path}.decision_signals.revision", errors
                )
                ...
                if (
                    ...
                    add(
                        errors,
                        f"{path}.decision_signals.margin",
                        "cannot exceed max_probability",
                    )
                ood_score = signals.get("ood_score")
                if ood_score is not None and (
                    not is_number(ood_score) or ood_score < 0
                ):
                    add(
                        errors,
                        f"{path}.decision_signals.ood_score",
...
def validate_run(
    ...
    for field in ("suite_ref", "task_ref", "entry_ref"):
        expect_ref(value.get(field), f"$.{field}", errors)
    instance = expect_fields(
        value.get("instance_set"),
        "$.instance_set",
        ("id", "revision", "sha256", "count"),
        errors,
    )
    if instance:
        ...
        if (
            not isinstance(instance.get("count"), int)
            or isinstance(instance.get("count"), bool)
            or instance.get("count", -1) < 1
        ):
    ...
    for field in ("started_at", "completed_at"):
        expect_string(value.get(field), f"$.{field}", errors)
    expect_enum(
        value.get("status"), contract["enums"]["run_status"], "$.status", errors
    )
...
def _validate_score_row(value: Any, path: str, errors: list[str]) -> None:
    ...
    if (
        ...
        or correct < 0
        or (isinstance(count, int) and correct > count)
    ):
        add(errors, f"{path}.correct", "must be an integer between zero and count")
    expected = (
        correct / count
        if isinstance(count, int) and count > 0 and isinstance(correct, int)
        else None
    )
    if expected is None:
        if accuracy is not None:
            add(errors, f"{path}.accuracy", "must be null when count is zero")
    elif not is_number(accuracy) or not math.isclose(
        accuracy, expected, rel_tol=0, abs_tol=1e-12
    ):
...
def validate_result(
    ...
    if instance:
        for field in ("id", "revision", "sha256"):
            expect_string(instance.get(field), f"$.instance_set.{field}", errors)
    counts = expect_fields(
        value.get("counts"),
        "$.counts",
        ("instances", "outputs", "correct", "incorrect", "errors"),
        errors,
    )
    if counts:
        ...
        if all(
            ...
            for field in ("correct", "incorrect", "errors", "outputs")
        ):
            if (
                counts["correct"] + counts["incorrect"] + counts["errors"]
                != counts["outputs"]
            ):
                add(
                    errors,
                    "$.counts",
                    "correct + incorrect + errors must equal outputs",
                )
    scores = expect_fields(
        value.get("scores"),
        "$.scores",
        ("exact_accuracy", "unknown_recall", "confusion_matrix"),
        errors,
    )
    if scores and counts and counts.get("outputs"):
        expected = counts.get("correct", 0) / counts["outputs"]
        if not is_number(scores.get("exact_accuracy")) or not math.isclose(
            scores["exact_accuracy"], expected, rel_tol=0, abs_tol=1e-12
        ):
            add(errors, "$.scores.exact_accuracy", "is inconsistent with counts")
        unknown_recall = scores.get("unknown_recall")
        if unknown_recall is not None and (
            not is_number(unknown_recall) or not 0 <= unknown_recall <= 1
        ):
        ...
        if not isinstance(confusion, dict) or not confusion:
            ...
            for expected_label, predicted_counts in confusion.items():
                if not isinstance(predicted_counts, dict) or not predicted_counts:
                    add(
                        errors,
                        f"$.scores.confusion_matrix.{expected_label}",
                ...
                for predicted_label, count in predicted_counts.items():
                    if (
                        ...
                        add(
                            errors,
                            f"$.scores.confusion_matrix.{expected_label}.{predicted_label}",
                            "must be a non-negative integer",
                        )
                    else:
                        matrix_total += count
            if matrix_total != counts["outputs"]:
                add(
                    ...
                    "counts must sum to output count",
                )
    ...
    reliability = expect_fields(
        value.get("reliability"),
        "$.reliability",
        ("repetitions", "consistent_instances", "consistency_rate"),
        errors,
    )
    if reliability:
        ...
        if (
            not isinstance(consistent, int)
            or isinstance(consistent, bool)
            or consistent < 0
            or (isinstance(total, int) and consistent > total)
        ):
            add(
                errors,
                "$.reliability.consistent_instances",
                "must be between zero and instance count",
            )
        expected = (
            consistent / total
            if isinstance(total, int) and total > 0 and isinstance(consistent, int)
            else None
        )
        if expected is not None and (
            not is_number(reliability.get("consistency_rate"))
            or not math.isclose(
                reliability["consistency_rate"], expected, rel_tol=0, abs_tol=1e-12
            )
        ):
            add(
                errors,
                "$.reliability.consistency_rate",
                "is inconsistent with consistent_instances/instances",
            )
    validate_measurements(value.get("resources"), contract, "$.resources", errors)
        ...
        if metrics:
            ...
            if resource_metrics is not None:
                resource_metrics = expect_fields(
                    ...
                    (
                        "latency_end_to_end_ms_mean",
                        "latency_end_to_end_ms_max",
                        "latency_cold_end_to_end_ms_mean",
                        "latency_warm_end_to_end_ms_mean",
                        "loaded_bytes_max",
                        "peak_resident_bytes",
                        "max_active_parameters",
                        "installed_bytes_touched_max",
                        "shared_base_bytes_touched_max",
                        "adapter_bytes_touched_max",
                        "external_calls",
                        "external_cost_usd",
                    ),
                    errors,
                ...
                if resource_metrics:
                    for field, item in resource_metrics.items():
                        if item is not None and (not is_number(item) or item < 0):
                            add(
                                errors,
                                f"$.system_metrics.resource_metrics.{field}",
                                "must be null or non-negative",
                            )
    ...
    if not isinstance(value.get("errors"), list) or any(
        not isinstance(item, dict) for item in value.get("errors", [])
    ):
        add(errors, "$.errors", "must be an array of objects")


def walk_receipt_privacy(
    value: Any, contract: dict[str, Any], path: str, errors: list[str]
) -> None:
...
def validate_receipt(
    ...
    expect_string(value.get("revision"), "$.revision", errors)
    expect_enum(
        value.get("evaluation_layer"),
        contract["enums"]["evaluation_layer"],
        "$.evaluation_layer",
        errors,
    )
    for field in (
    ...
    if frontier:
        expect_enum(
            frontier.get("state"),
            contract["enums"]["frontier_state"],
            "$.frontier_qualification.state",
            errors,
        )
    leakage = expect_fields(
        value.get("leakage"),
        "$.leakage",
        ("permitted_training_cutoff", "overlap_check", "overlap_count"),
        errors,
    )
    if leakage:
        expect_string(
            leakage.get("permitted_training_cutoff"),
            "$.leakage.permitted_training_cutoff",
            errors,
        )
        expect_string(leakage.get("overlap_check"), "$.leakage.overlap_check", errors)
        if (
            not isinstance(leakage.get("overlap_count"), int)
            or isinstance(leakage.get("overlap_count"), bool)
            or leakage.get("overlap_count", -1) < 0
        ):
            add(errors, "$.leakage.overlap_count", "must be a non-negative integer")
    custody = expect_fields(
        value.get("custody"),
        "$.custody",
        ("holder", "instance_material_committed", "replay_authority"),
        errors,
    )
    if custody:
        expect_string(custody.get("holder"), "$.custody.holder", errors)
        expect_bool(
            custody.get("instance_material_committed"),
            "$.custody.instance_material_committed",
            errors,
        )
        expect_string(
            custody.get("replay_authority"), "$.custody.replay_authority", errors
        )
        if value.get("evaluation_layer") == "sealed-official" and custody.get(
            "instance_material_committed"
        ):
            add(
                errors,
                "$.custody.instance_material_committed",
                "must be false for sealed official evaluation",
            )
    aggregate = expect_fields(
        value.get("aggregate"),
        "$.aggregate",
        (
            "exact_accuracy",
    ...
    if aggregate:
        ...
        expect_string(
            aggregate.get("result_sha256"), "$.aggregate.result_sha256", errors
        )
    attestation = expect_fields(
        value.get("attestation"), "$.attestation", ("kind", "value"), errors
    )
    if attestation:
        expect_string(attestation.get("kind"), "$.attestation.kind", errors)
        expect_string(
            attestation.get("value"), "$.attestation.value", errors, nullable=True
        )
    expect_enum(
        value.get("publication_authority"),
        contract["enums"]["publication_authority"],
    ...
    if len(raw) > contract["limits"]["receipt_bytes"]:
        add(
            errors,
            "$",
            f"receipt exceeds {contract['limits']['receipt_bytes']} byte limit",
        )
...
def validate_artifact(
    value: Any, contract: dict[str, Any], errors: list[str], *, source: str = "$"
) -> None:
    ...
    if obj.get("contract_version") != contract.get("contract_version"):
        add(
            errors,
            f"{source}.contract_version",
            f"must be {contract.get('contract_version')!r}",
        )
...
def validate_bundle(
    values: list[dict[str, Any]], contract: dict[str, Any], errors: list[str]
) -> None:
    ...
    instance_sets = {
        _artifact_ref(item): item for item in by_type.get("instance_set", [])
    }
    for prediction in by_type.get("prediction_set", []):
        refs = (
            ("task_ref", tasks),
            ("entry_ref", entries),
            ("instance_set_ref", instance_sets),
        )
        for field, index in refs:
            ...
            if index and key not in index:
                add(
                    errors,
                    f"prediction_set:{prediction.get('prediction_set_id')}.{field}",
                    f"dangling reference {key}",
                )
        instance_ref = prediction.get("instance_set_ref", {})
        instance_set = instance_sets.get(
            (instance_ref.get("id"), instance_ref.get("revision"))
        )
        if instance_set:
            expected_ids = {
                item.get("id") for item in instance_set.get("instances", [])
            }
            output_ids = {
                item.get("instance_id") for item in prediction.get("outputs", [])
            }
            unknown_ids = sorted(output_ids - expected_ids)
            if unknown_ids:
                add(
                    errors,
                    f"prediction_set:{prediction.get('prediction_set_id')}.outputs",
                    f"unknown instance ids: {', '.join(unknown_ids)}",
                )
        ...
        if task and instance_set:
            ...
            if any(
                expected_field not in item for item in instance_set.get("instances", [])
            ):
                add(
                    errors,
                    f"instance_set:{instance_set.get('instance_set_id')}.instances",
                    f"missing task expected field {expected_field!r}",
                )
            if any(
                ...
                add(
                    errors,
                    f"prediction_set:{prediction.get('prediction_set_id')}.outputs",
                    f"missing task prediction field {prediction_field!r}",
                )
    ...
    for key, identities in result_groups.items():
        if len(identities) > 1:
            add(
                errors,
                "bundle.results",
                f"headline-incompatible instance sets for suite/task {key}: {sorted(identities)}",
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "paths", nargs="+", type=Path, help="JSON artifacts to validate together"
    )
    ...
    validate_bundle(
        [value for value in values if isinstance(value, dict)], contract, errors
    )
    if errors:
        for error in errors:
        ...
        return 1
    print(
        f"everyday-benchmark: validated {len(values)} artifact(s) against {contract['contract_version']}"
    )
    return 0
