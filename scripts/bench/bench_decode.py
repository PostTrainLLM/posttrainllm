...
DEFAULT_SYS_PROMPT = "You are a helpful assistant. Answer concisely."
DEFAULT_USER_PROMPT = (
...
def one_run(
    ...
    if t_first is None:
        ...
        raise RuntimeError(
            "no tokens streamed; is --url an OpenAI-compat "
...
def main():
    ...
    ap.add_argument(
        ...
        default=os.environ.get(
            "BENCH_URL", "http://127.0.0.1:1234/v1/chat/completions"
        ),
    ...
    ap.add_argument("--n", type=int, default=20, help="measured runs (default 20)")
    ap.add_argument(
        "--warm",
        type=int,
        default=3,
    ...
    ap.add_argument(
        "--jsonl", default=None, help="append EvalHarnessSupport-shaped rows here"
    )
    ap.add_argument("--label", default=None, help="row label (default: --model)")
    args = ap.parse_args()
    ...
    for i in range(args.n):
        ...
        print(
            f"  run {i + 1}/{args.n}: ttft={r['ttft_ms']:.1f}ms  "
            f"n={r['n_tokens']}  "
