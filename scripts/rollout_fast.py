...
def cleanup(
    model_name,
):  # drop this rollout's isolated BFCL instances from module globals
    for k in [
        k for k in list(mtu.__dict__) if model_name in k and k.endswith("_instance")
    ]:
        del mtu.__dict__[k]
...
class Roll:
    ...
    def uturn(self, i):
        return " ".join(
            m["content"] for m in self.ex["question"][i] if m.get("role") == "user"
        )
...
def end_turn(r):
    ...
    if r.ti < len(r.ex["question"]):
        r.messages.append({"role": "user", "content": r.uturn(r.ti)})
        r.turn_steps = []
        r.step = 0
    else:
        try:
            res = multi_turn_checker(
                r.decoded, r.gold["ground_truth"], r.ex, CAT, r.name
            )
...
def main():
    ...
    for ex in data:
        ...
        for _ in range(ROLLOUTS):
            rolls.append(Roll(ex, g, tools, idx))
            idx += 1
    ...
    t0 = time.time()
    print(
        f"FAST ROLLOUT  tasks={n_tasks} rollouts={ROLLOUTS} temp={TEMP} batch={BATCH}"
    ...
    while True:
        ...
        prompts = [
            _tok.apply_chat_template(
                r.messages, tools=r.tools, add_generation_prompt=True, tokenize=True
            )
            for r in active
        ]
        ...
        for r, out in zip(active, texts):
            ...
            if r.status == "done" and r.valid and r.ex["id"] not in solved:
                ...
                if dumpf:
                    dumpf.write(
                        json.dumps(
                            {"id": r.ex["id"], "tools": r.tools, "messages": r.messages}
                        )
                        + "\n"
                    )
                    dumpf.flush()
        print(
            f"  round {rnd}: active={len(active)} solved_tasks={len(solved)}/{n_tasks} "
    ...
    if dumpf:
        dumpf.close()
    print(
        f"== FAST ROLLOUT: {len(solved)}/{n_tasks} tasks solved (>=1 win in {ROLLOUTS}), "
