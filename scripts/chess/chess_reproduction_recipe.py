...
def load_recipe(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or recipe.get("schema_version") != SCHEMA_VERSION
        or set(recipe) != required
    ):
    ...
    if not isinstance(stages, list) or [row.get("total_rows") for row in stages] != [
        10000,
        100000,
        1000000,
        2000000,
    ...
    for stage in stages:
        ...
        if set(stage["arms"]) != {"terse", "commentary-8pct"}:
            raise ValueError(
                "every reproduction stage needs matched terse and commentary arms"
            )
        for arm_name, counts in stage["arms"].items():
            if set(counts) != SOURCE_NAMES or any(
            ...
            if sum(counts.values()) != stage["total_rows"]:
                raise ValueError(
                    f"{stage['stage_id']} {arm_name} counts do not sum to total_rows"
                )
        commentary = stage["arms"]["commentary-8pct"]["grounded_commentary"]
        if commentary * 100 != stage["total_rows"] * 8:
            raise ValueError(
                "commentary arm must contain exactly eight percent grounded commentary"
            )
    ...
    if not evaluation.get("rerate_after_any_policy_change") or not evaluation.get(
        "report_guard_fire_rate"
    ):
        raise ValueError(
            "reproduction evaluation must rerate policy changes and report guard fire rate"
        )
    return recipe
