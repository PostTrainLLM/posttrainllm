...
class CapabilityGraphTests(unittest.TestCase):
    def setUp(self):
        ...
        self.policy = load(
            ROOT / "configs/capability-graph/policies/local-only-v1.json"
        )
    ...
    def test_development_graph_and_policy_are_valid(self):
        ...
        referenced = {
            item["package_id"] for item in self.graph["nodes"] if item["package_id"]
        }
    ...
    def test_validator_rejects_duplicate_and_dangling_ids(self):
        ...
        dangling = self.mutate(
            lambda graph: graph["edges"][0].update({"to": "missing-node"})
        )
    ...
    def test_validator_rejects_cycles_schema_mismatch_and_unverified_acceptance(self):
            ...
            graph["edges"].append(
                {
                    "id": "cycle-back",
                    "kind": "fallback-to",
                    "from": "local-generalist-placeholder",
                    "to": "file-ops-distilled",
                    "order": 2,
                    "capability": "file-ops",
                    "request_schema": "request/text-v1",
                    "response_schema": "file-plan/v1",
                    "executable": True,
                }
            )

        cycle = self.mutate(add_cycle)
        mismatch = self.mutate(
            lambda graph: graph["edges"][0].update({"response_schema": "wrong/v1"})
        )
        ...
        self.assertTrue(
            any("target schema is incompatible" in error for error in mismatch)
        )
        self.assertTrue(any("no valid verifier edge" in error for error in unverified))

    def test_validator_rejects_missing_safe_terminal_unsupported_claims_and_private_data(
        self,
    ):
        def remove_terminal(graph):
            graph["edges"] = [
                edge
                for edge in graph["edges"]
                if edge["id"] != "fallback-general-external"
            ]
            next(
                node
                for node in graph["nodes"]
                if node["id"] == "local-generalist-placeholder"
            )["terminal_behavior"] = "fallback-only"

        terminal = self.mutate(remove_terminal)
        unsupported = self.mutate(
            lambda graph: graph["nodes"][1]["verifier_policy"].update(
                {"rule": "accept self-confidence"}
            )
        )
        private = self.mutate(
            lambda graph: graph.update({"api_key": "sk-test-12345678901234567890"})
        )
        self.assertTrue(
            any("no fallback or safe terminal" in error for error in terminal)
        )
        self.assertTrue(
            any("self-confidence cannot accept" in error for error in unsupported)
        )
    ...
    def test_dry_run_is_deterministic_and_selects_smallest_eligible_node(self):
        first = capability.dry_run(
            self.graph, self.policy, self.request, self.installed, self.router
        )
        second = capability.dry_run(
            self.graph, self.policy, self.request, self.installed, self.router
        )
    ...
    def test_low_confidence_bypasses_specialists(self):
        ...
        route = capability.dry_run(
            self.graph, self.policy, self.request, self.installed, router
        )
    ...
    def test_residency_never_overrides_capability_privacy_or_quality(self):
        ...
        for mutation in mutations:
            graph = copy.deepcopy(self.graph)
            node = next(
                item for item in graph["nodes"] if item["id"] == "file-ops-distilled"
            )
            mutation(node)
            route = capability.dry_run(
                graph, self.policy, self.request, self.installed, self.router
            )
            self.assertNotIn("file-ops-distilled", route["eligible"])

    def test_verifier_acceptance_and_rejection_fallback(self):
        accepted = capability.cascade(
            self.graph,
            self.policy,
            self.request,
            self.installed,
        ...
        outcomes["file-ops-distilled"]["verifier"] = {
            "accepted": False,
        ...
        fallback = capability.cascade(
            self.graph, self.policy, self.request, self.installed, self.router, outcomes
        )
        self.assertEqual(fallback["result"]["node_id"], "file-ops-rest")
        self.assertEqual(
            fallback["trace"]["attempts"][0]["failure"], "verifier-rejected"
        )
    ...
    def test_timeout_load_failure_and_exhaustion_return_safe_failure(self):
        ...
        execution = capability.cascade(
            self.graph, self.policy, self.request, self.installed, self.router, outcomes
        )
        self.assertEqual(
            execution["result"],
            {"kind": "safe-failure", "failure": "no-accepted-result"},
        )
    ...
    def test_hop_latency_and_external_cost_budgets_are_enforced(self):
        ...
        execution = capability.cascade(
            self.graph, hop_policy, self.request, self.installed, self.router, {}
        )
        ...
        external = next(
            node
            for node in graph["nodes"]
            if node["id"] == "external-frontier-disabled"
        )
        ...
        policy.update(
            {
                ...
                "max_external_cost_usd": 0.01,
            }
        )
        request = copy.deepcopy(self.request)
        request["external_authorized"] = True
        router = {
            "ranked_node_ids": ["external-frontier-disabled"],
            "confidence": 1,
            "ood_score": 0,
        }
        outcome = {
            "external-frontier-disabled": {
                ...
                "verifier": {"accepted": True, "reason": "pass"},
            }
        }
        execution = capability.cascade(
            graph, policy, request, self.installed, router, outcome
        )
        self.assertEqual(
            execution["result"]["failure"], "external-cost-budget-exhausted"
        )
    ...
    def test_external_execution_requires_all_three_opt_ins(self):
        ...
        blocked = capability.dry_run(
            graph, self.policy, self.request, self.installed, router
        )
    ...
    def test_everyday_benchmark_adapter_binds_graph_policy_nodes_and_resources(self):
        ...
        entry = load(
            ROOT
            / "configs/everyday-benchmark/entries/pace-intent-v8-qwen-cascade-v1.json"
        )
        prediction = capability.benchmark_prediction(
            execution, task, entry, "fileops-public-001"
        )
        output = prediction["outputs"][0]
        metrics = everyday.system_metrics(
            prediction["outputs"], {"fileops-public-001": "pass"}
        )
        self.assertEqual(metrics["false_accept_rate"], 0)
        self.assertEqual(metrics["resource_metrics"]["latency_end_to_end_ms_mean"], 12)
        self.assertEqual(
            metrics["resource_metrics"]["latency_warm_end_to_end_ms_mean"], 12
        )
        self.assertIsNone(
            metrics["resource_metrics"]["latency_cold_end_to_end_ms_mean"]
        )
        self.assertEqual(
            metrics["resource_metrics"]["installed_bytes_touched_max"], 8044981893
        )
        self.assertEqual(
            metrics["resource_metrics"]["shared_base_bytes_touched_max"], 8044981893
        )
        self.assertEqual(metrics["resource_metrics"]["adapter_bytes_touched_max"], 0)
        self.assertEqual(
            output["routing"]["binding"]["invoked_node_ids"], ["file-ops-distilled"]
        )
        self.assertEqual(
            output["routing"]["binding"]["invoked_package_ids"],
            ["qwen3-4b-file-ops-distilled"],
        )
        self.assertEqual(
            output["routing"]["binding"]["verifier_ids"], ["file-state-verifier"]
        )
        self.assertEqual(
            output["routing"]["binding"]["trace_contract_version"],
            "specialist-capability-trace/v1",
        )
        self.assertEqual(
            output["routing"]["resource_evidence"]["peak_resident_bytes"], 8044981893
        )
    ...
    def test_system_qualification_fails_closed_on_router_or_end_to_end_gates(self):
        ...
        self.assertEqual(
            capability.system_qualification(1, qualified_metrics, gates)["state"],
            "qualified",
        )
        ...
        leaf_strong_but_system_failed = capability.system_qualification(
            0.8, qualified_metrics, gates
        )
        self.assertEqual(leaf_strong_but_system_failed["state"], "unqualified")
        self.assertFalse(
            leaf_strong_but_system_failed["checks"]["final_accuracy"]["passed"]
        )
