...
- Training data: teacher-free ReST — checker-passing trajectories generated by
  the candidate itself over repository-authored tasks, plus a file-ops gold
  depth anchor. No external teacher outputs are embedded in the training set.
- The distributed artifact is itself fused Qwen-derived weights; no BFCL
  datasets or additional third-party weights are bundled. The evaluation
  methodology derives from Berkeley's BFCL multi-turn suite (Apache-2.0).
