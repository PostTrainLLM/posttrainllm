...
description: "posttrainllm (the factory) refocuses on local-LLM research. This doc freezes the"
---

# Pace handoff — posttrainllm deliverables status (2026-06-10)

posttrainllm (the factory) refocuses on local-LLM research. This doc freezes the
state of everything posttrainllm owes or has delivered to Pace (the consumer),
...
|---|---|---|
| v9 planner LoRA (shipping specialist) | `~/.cache/posttrainllm/runs/pace-planner-v9/` | 33.3% v2 / 70% compose; **not** Pace's runtime default (see below) |
| ANE M8 28-block chain | posttrainllm ANE scripts | 17 tok/s, ' Paris' top-1 correct |
...
- **Important reframe**: Pace's runtime planner default is currently `qwen/qwen3-30b-a3b` via LM Studio (set in Pace's Info.plist), NOT the posttrainllm specialist. v11's real bar is beating the 30B on (speed × accuracy) / cost, not just beating v9. Add a 30B comparison row to the eval before deciding ship.

## Owed — open posttrainllm work Pace is waiting on
...
| #292-adj | WhisperKit streaming runtime wiring | Pace's `TranscriptionProvider: whisperKit` scaffold falls back to Apple Speech | mostly Pace-side; posttrainllm advises |
...
- Another agent works in pace repo; posttrainllm-side work avoids contention
