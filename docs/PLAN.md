...
| Script                        | Role                                                                                      |
| ----------------------------- | ----------------------------------------------------------------------------------------- |
| `scripts/pipelines/score-checkpoint.sh` | one `.tinygpt` → E0 JSONL row(s) via lm-eval                                              |
| `scripts/pipelines/score-run.sh`        | every history checkpoint of a run + SmolLM2 baseline → JSONL + 3-view summary             |
| `scripts/sae-run.sh`          | SAE-per-checkpoint sweep → JSONL timeline (B13 v2 input)                                  |
| `scripts/pipelines/score-baselines.sh`  | 5 HF baselines (SmolLM2-135/360M, Qwen3-0.6B, TinyLlama, Phi-3-mini) on the same task set |
...
- ⏸ **B14a. Serve FSM trie masking optimization** (PARKED) — a Swift `Dictionary`-backed trie was tried and did not beat the legacy masker in measured decode latency, so the legacy path remains the default. Revisit only with bit-exact differential tests for representative prompts/FSM states plus first-token, steady-state tok/s, and masker-only benchmarks that prove a win. Prefer compact array/struct-of-arrays transitions, precomputed token-class bitsets, or work moved out of the per-token hot path over another object-heavy trie.
