...
- 🟡 **B23. Agent eval protocol hardening** — statistical-reporting slice shipped 2026-06-18: `tinygpt eval-gate --passes K` now gates on K-pass means and preserves per-trial scores, stdev, stderr, and 95% CI in `gate-result.json`; the console renders `mean±ci95`. Remaining: declarative budget config/logging for BFCL, τ-bench, Pace unhappy paths, and future SWE-mini/Terminal-mini so fixed max steps, sandbox resources, sampling params, and infra patches travel with every score. PRD: `docs/prds/B23-agent-eval-protocol.md`.
...
- 🟡 **B32. `tinygpt eval` as a CI / pre-commit gate** (shipped 2026-06-13; K-pass stats added 2026-06-18; live multi-suite GPU run pending a self-hosted runner) — `tinygpt eval-gate` runs declared suites vs a baseline and exits non-zero on regression. Pure gate logic in `TinyGPTModel/EvalGate.swift` (direction heuristic, pp thresholds, per-suite override, missing-baseline handling, K-pass mean + stdev/stderr/95% CI) with unit tests; CLI orchestration in `Sources/TinyGPT/EvalGate.swift` (`--candidate` no-GPU path, `--update-baseline`, `--passes`, `gate-result.json`). Spec lives in `eval-gate.json` or the `tinygpt.project.json` `eval` block (B31 schema add). GitHub Action `.github/actions/tinygpt-eval-gate/`, recipe `docs/recipes/eval-gate.md`, smoke `evals/eval-gate-smoke.sh` (asserts exit 0 match, exit 1 regression, and repeated-run stats). **Flips to ✅** once a real specialist's suites run end-to-end through the gate on a self-hosted Mac runner. PRD: `docs/prds/B32-eval-ci-gate.md`.
