...
related_prds: B22-token-preserving-agent-trajectory.md, B26-deferred-tools.md,
              ...
              B32-eval-ci-gate.md
context: docs/sessions/2026-06-17-vercel-eve-and-local-agents.md (TBD)
---
...
a code reviewer that runs entirely on the user's Mac on a tinygpt
...
See [[feedback_tinygpt_north_star]] — formula is (speed × accuracy) /
...
- Approach: QLoRA + composite reward (B28 dims: identifies-bug,
  ...
  `tinygpt sft` then DPO on (preferred-review, dispreferred-review) pairs.
...
- Thin agent harness: `tinygpt agent` subcommand, with the verb model
...
- **No new infra** — reuse `tinygpt serve` for the runtime, B26 for the
...
- [ ] tinygpt-specialist beats open-baseline by ≥ 5pp on accuracy
      (matches the [[feedback_tinygpt_north_star]] 5%/2-weeks bar).
- [ ] Specialist runs end-to-end on M5 Pro / 48 GB at ≥ 15 tok/s
      decode (informal target — the bar is "feels responsive").
- [ ] One real codebase (TinyGPT itself? Or a public OSS repo) has a
...
the negative result and keep tinygpt narrow as a model factory, not an
