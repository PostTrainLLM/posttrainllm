...
project**. The ground-up [`curriculum.md`](curriculum.md) is the _spine_ (10
modules, first principles to self-improving factory). This page is the _net_:
...
The spine teaches you to _read_ the whole system; this map is the index that
proves nothing was left unread.

The runnable lab graph lives in
[`path-registry.json`](path-registry.json). It gives every path prerequisites,
an ordered sequence, repository anchors, a hands-on lab, a mastery gate, and
validated CLI entry points. The public `/learn` cards also resolve the recipes
assigned to each path and expose every evidence anchor rather than only the
first guide.
The public walkthrough at [`/learn`](https://posttrainllm.com/learn) renders the
same paths, while [`../recipes/registry.json`](../recipes/registry.json) maps
every retained technique recipe back to one of them.
...
For this closed historical project, the validator additionally requires every
recipe ID to resolve to one of the path IDs in `path-registry.json`. Fresh
experiments may extend the graph only after the owner starts a new question.
