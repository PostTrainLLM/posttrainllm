...
export const artifacts: ArtifactEntry[] = [
  ...
  },
  {
    ...
    eyebrow: "Reference-scored browser ASR",
    state: "report-only",
    date: "2026-09-02",
    kind: "Paired WebGPU and native ASR evaluation",
    ...
    lede: "This closes the accuracy gap left by the original long-form smoke. On the frozen 34.365-second fixture, the official browser engine beat the native incumbent on WER, matched all four proper nouns, produced no repetition errors, and made no external warm requests. Its 33.84x median real-time factor is strong but below the preregistered 50x bar, so the formal decision remains reject rather than moving the goalposts.",
    metrics: [
      {
        label: "Browser WER",
        value: "0.0%",
        context: "0/82 errors · eight test-clean clips",
        evidence: "measured",
      },
      {
        label: "Native WER",
        value: "8.54%",
        context: "7/82 errors · identical clips and references",
        evidence: "measured",
      },
      {
        label: "Paired latency",
        value: "3.51x",
        context: "914.7ms browser · 3211.8ms native",
        evidence: "measured",
      },
      {
        label: "Short-clip RTFx",
        value: "33.84x",
        context: "median · frozen gate required ≥50x",
        evidence: "measured",
      },
    ],
    comparisons: [
      {
        name: "Parakeet TDT 0.6B v3 · browser",
        metric: "WER · total decode",
        score: "0.0% · 914.7ms",
        size: "684.4MB first download",
        comparability: "Direct",
        note: "Apple Metal WebGPU; zero warm external requests.",
      },
      {
        name: "WhisperKit Mac-native baseline",
        metric: "WER · total decode",
        score: "8.54% · 3211.8ms",
        size: "1.5GB local Core ML model",
        comparability: "Direct",
        note: "Whisper large-v3 turbo; CPU and Neural Engine.",
      },
    ],
    tables: [
      {
        title: "Frozen promotion gates",
        columns: ["Gate", "Result", "Decision"],
        rows: [
          ["WER delta ≤2 points", "−8.54 points", "pass"],
          ["Median warm RTFx ≥50", "33.84x", "fail"],
          ["No repetition regression", "0 vs 0", "pass"],
          ["Warm external requests", "0", "pass"],
        ],
      },
    ],
    evidence: [
      {
        label: "Paired result",
        href: "https://github.com/PostTrainLLM/posttrainllm/blob/main/evals/verified-wins/parakeet-asr-result-v1.json",
      },
      {
        label: "Experiment protocol and interpretation",
        href: "/docs/techniques/parakeet-wgsl-browser-smoke",
      },
      {
        label: "Frozen manifest",
        href: "https://github.com/PostTrainLLM/posttrainllm/blob/main/evals/verified-wins/parakeet-asr-v1.json",
      },
      { label: "Complete experiment archive", href: "/experiments" },
    ],
    blockers: [
      {
        blocker: "50x short-clip gate missed",
        why: "The 2–7 second clips expose an approximately 80–100ms browser dispatch floor; median throughput reached 33.84x despite beating native latency by 3.51x.",
        unblock:
          "Do not change this decision. A separately frozen batching or long-form latency experiment may test whether fixed overhead amortizes.",
      },
      {
        blocker: "Large optional first download",
        why: "The pinned v3 browser model downloaded 684,385,250 bytes.",
        unblock:
          "Any product use must keep the download explicit, optional, and measured on Chrome and Safari.",
      },
    ],
    nextAction:
      "Closed as a quality and native-latency win with a formal speed-threshold miss. Use it to teach paired evaluation, fixed-cost amortization, and why frozen gates matter.",
  },
  {
