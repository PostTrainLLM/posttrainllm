...
| Date / phase | Status |
|---|---|
| 2026-07-25 Autocorrect foundation and calibration | Completed the bounded no-local-model tranche of `build-mac-local-autocorrect-specialist`: correction protocol, quality/resource gates, 18-row original MIT-licensed smoke ruler, strict evaluator, leakage/provenance checks, deterministic Mac keyboard simulator, tiny/pilot manifests, distribution report, and non-equivalent Apple assessment. Codex CLI `gpt-5.6-sol` independently matched all 18 rows with perfect clean/protected-span preservation; the hash-linked result establishes fixture clarity, not population representativeness. Pinned three Apache-2.0 base candidates with exact download commands and conservative resource/cleanup estimates. The 13-test smoke plus strict OpenSpec validation pass. No local weights were downloaded or loaded, no dependencies installed, and no model trained, packaged, or shipped. |
...
- The no-model foundation of
  ...
  calibration (task 2.5) and the pinned three-base research (task 4.1) are
  complete. Tasks 4.2-7 remain gated on immediate approval for the recorded
  downloads, dependency installation, model loading, compilation, GPU/training,
  decoding/performance measurement, and final packaging. The committed 18-row
  fixture is an unambiguous smoke ruler, not representative natural-error
  evidence, and no local candidate or ship claim exists.
