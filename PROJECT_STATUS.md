...
| Date / phase | Status |
|---|---|
| 2026-07-25 Autocorrect no-model foundation | Completed the bounded foundation tranche of `build-mac-local-autocorrect-specialist` without model/network/GPU work: committed v1 correction protocol, error/failure taxonomy, numeric quality/resource/energy/stop gates, 18 reviewed original MIT-licensed smoke rows, strict stdlib evaluator, source-first split/normalized-overlap/lexical-holdout/provenance/hash validator, versioned Mac US ANSI layout, deterministic seven-family corruption simulator with edit traces, 3.3 KB tiny-overfit and 7.0 KB pilot manifests, and a frozen natural-vs-synthetic distribution report. Local macOS 27 AppKit SDK inspection classifies Apple autocorrect as an observational non-equivalent baseline, not a direct full-span comparator. `evals/autocorrect-foundation-smoke.sh` covers 12 focused tests plus strict artifact/evaluator checks. Remaining OpenSpec work is explicitly blocked on Codex frontier calibration and approved model research/download/load/training/decoding/performance runs; no candidate, package, or ship claim exists. |
...
- The no-model foundation of
  `openspec/changes/build-mac-local-autocorrect-specialist` is complete and
  documented in `docs/factory/autocorrect-foundation.md`. Task 2.5 remains
  blocked on an explicitly prohibited Codex/frontier calibration call. Tasks
  4-7 remain blocked on current model research plus separate approval for
  downloads, model loading, compilation, GPU/training, decoding/performance
  measurement, and final packaging. The committed 18-row fixture is a smoke
  ruler, not representative natural-error evidence, and no candidate or ship
  claim exists.
