...
| Date / phase | Status |
|---|---|
| 2026-07-11 reference-anchored DPO retry | Executed end-to-end on GPU: reference-anchored DPO (`--loss-type dpo`, 50 steps, lr 5e-6) on the frozen qwen06-sql-hygiene pairs. **Collapse fixed** — composed execution 0.860 → 0.900 (SimPO retry was 0.080), DPO-alone healthy at 0.120, step-1 loss 0.6931 ≈ log 2. Decision **retry-training**: hygiene bar unmet (clean-SQL 0.000; `Answer:/Explanation:` wrapper persists). Run assembled via `scripts/assemble_factory_run.py` (validates + publish-check). Run: `runs/2026-07-11-sql-hygiene-dpo-refanchored-qwen06/` (gitignored). Second full factory loop with a real decision. |
...
| TinyGPTApp | Useful shell over CLI. Parked except for a minimal factory readout/control surface after CLI proof. |
